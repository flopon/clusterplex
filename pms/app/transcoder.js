...
if (TRANSCODE_OPERATING_MODE == "local") {
        ...
        console.error("EasyAudioEncoder used, forcing local transcode");
        transcodeLocally(process.cwd(), process.argv.slice(2), process.env);
} else if (process.cwd().indexOf("PlexCreditsDetection") > 0) {
        console.error("PlexCreditsDetection so forcing local transcode");
        transcodeLocally(process.cwd(), process.argv.slice(2), process.env);
} else {
        ...
        if (LOCAL_RELAY_ENABLED == "1") {
                ...
                if (FORCE_HTTPS == "1") {
                        console.error("Forcing HTTPS in progress callback");
        ...
        if (TRANSCODER_VERBOSE == "1") {
                console.error("Setting VERBOSE to ON");
        ...
        console.error(`Sending request to orchestrator on: ${ORCHESTRATOR_URL}`);
        if (TRANSCODER_VERBOSE == "1") {
                console.error(`cwd => ${JSON.stringify(workDir)}`);
                console.error(`args => ${JSON.stringify(newArgs)}`);
                console.error(`env => ${JSON.stringify(environmentVariables)}`);
        }
        ...
        jobPoster.postJob(
                ...
                (response) => {
                        if (!response.result) {
                                ...
                                console.error("Remote Transcoding was successful");
                                process.exit(0);
                        }
                }
        );
...
function transcodeLocally(cwd, args, env) {
        ...
        child.on("close", (code) => {
                console.error("Completed local transcode");
                process.exit(withErrors);
        });
}

ON_DEATH((signal, err) => {
        console.error("ON_DEATH signal detected");
        console.error(err);
        if (child) {
                console.error("Killing child transcoder");
                child.kill();
        }
        let exitCode = 0;
        switch (signal) {
